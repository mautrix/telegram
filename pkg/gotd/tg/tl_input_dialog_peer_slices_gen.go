...
// AsInputDialogPeerCommunity returns copy with only InputDialogPeerCommunity constructors.
func (s InputDialogPeerClassArray) AsInputDialogPeerCommunity() (to InputDialogPeerCommunityArray) {
	for _, elem := range s {
		value, ok := elem.(*InputDialogPeerCommunity)
		if !ok {
			continue
		}
		to = append(to, *value)
...
// InputDialogPeerCommunityArray is adapter for slice of InputDialogPeerCommunity.
type InputDialogPeerCommunityArray []InputDialogPeerCommunity

// Sort sorts slice of InputDialogPeerCommunity.
func (s InputDialogPeerCommunityArray) Sort(less func(a, b InputDialogPeerCommunity) bool) InputDialogPeerCommunityArray {
	sort.Slice(s, func(i, j int) bool {
		return less(s[i], s[j])
	})
	return s
...
// SortStable sorts slice of InputDialogPeerCommunity.
func (s InputDialogPeerCommunityArray) SortStable(less func(a, b InputDialogPeerCommunity) bool) InputDialogPeerCommunityArray {
	sort.SliceStable(s, func(i, j int) bool {
		return less(s[i], s[j])
	})
	return s
...
// Retain filters in-place slice of InputDialogPeerCommunity.
func (s InputDialogPeerCommunityArray) Retain(keep func(x InputDialogPeerCommunity) bool) InputDialogPeerCommunityArray {
	n := 0
	for _, x := range s {
		if keep(x) {
			s[n] = x
...
// First returns first element of slice (if exists).
func (s InputDialogPeerCommunityArray) First() (v InputDialogPeerCommunity, ok bool) {
	if len(s) < 1 {
		return
	}
	return s[0], true
...
// Last returns last element of slice (if exists).
func (s InputDialogPeerCommunityArray) Last() (v InputDialogPeerCommunity, ok bool) {
	if len(s) < 1 {
		return
	}
	return s[len(s)-1], true
...
// PopFirst returns first element of slice (if exists) and deletes it.
func (s *InputDialogPeerCommunityArray) PopFirst() (v InputDialogPeerCommunity, ok bool) {
	if s == nil || len(*s) < 1 {
		return
	}
	...
	// Delete by index from SliceTricks.
	copy(a[0:], a[1:])
	var zero InputDialogPeerCommunity
	a[len(a)-1] = zero
	a = a[:len(a)-1]
	*s = a
...
// Pop returns last element of slice (if exists) and deletes it.
func (s *InputDialogPeerCommunityArray) Pop() (v InputDialogPeerCommunity, ok bool) {
	if s == nil || len(*s) < 1 {
		return
	}
