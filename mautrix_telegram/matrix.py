...
from typing import Dict, Optional, Set, Tuple, Union, Iterable, TYPE_CHECKING
...
class MatrixHandler(BaseMatrixHandler):
    ...
    def __init__(self, context: 'Context') -> None:
        ...
        self.management_room_multiple_messages = context.config["bridge.management_room_multiple_messages"] or False
    ...
    async def send_welcome_message(self, room_id: RoomID, inviter: 'u.User') -> None:
        ...
        if not is_management or not inviter.puppet_whitelisted:
        ...
        # Get welcome text
        welcome_text, welcome_plain = self._get_welcome_message_string("welcome", text)
        if self.management_room_multiple_messages:
            await self.az.intent.send_notice(room_id, text=welcome_text, html=welcome_plain)
        else:
            text += welcome_text, welcome_plain

        if await inviter.is_logged_in():
            logged_in_text, logged_in_html = self._get_welcome_message_string(
                "welcome_connected",
                f"Use `help` for help.",
                f"Use <code>help</code> for help."
            )
            if self.management_room_multiple_messages:
                await self.az.intent.send_notice(room_id, text=logged_in_text, html=logged_in_html)
            else:
                text += logged_in_text
                html += logged_in_html
        else:
            ...
            text += unconnected_text
            html += unconnected_html
            if self.management_room_multiple_messages:
                await self.az.intent.send_notice(room_id, text=unconnected_text, html=unconnected_html)
            else:
                text += unconnected_text
                html += unconnected_html

        additional_help_text, additional_help_html = self._get_welcome_message_string("additional_help", "")

        if additional_help_text:
            if self.management_room_multiple_messages:
                await self.az.intent.send_notice(room_id, text=additional_help_text, html=additional_help_html)
            else:
                text += additional_help_text
                html += additional_help_html
